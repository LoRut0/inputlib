...
double Input::double_(double min, double max, unsigned max_after_comma)
{
	if (min > max) {
		std::cout << "\nERROR: min > max" << std::endl;
		abort();
	...
	while (true)
	{
		...
		if (isdigit(get))
		{
			if (size_check(future_number + char(get), min, max, case_event)) continue;

			size_t comma_place = future_number.find(',');
			//if comma is there and max_after_comma_check true
			if ((comma_place != std::string::npos) and (max_after_comma_check(future_number + char(get), comma_place, max_after_comma))) continue;

			future_number.push_back(char(get));
			std::cout << char(get);
		}
		//minus input if min < 0
		if ((get == 45) and (future_number.length() == 0) and (min < 0))
		{
			future_number.push_back('-');
			std::cout << char(get);
		}
		if ( ((get == '.') and (future_number.find(',') == std::string::npos)) and ((future_number.length() >= 1 and future_number[0] != '-') or (future_number.length() >= 2)) ) {
			future_number.push_back(',');
			std::cout << char(get);
		}
		if (get == 8) back(future_number);
